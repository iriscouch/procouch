...
exports.heat = function(opts) {
  ...
  update_db({seq:0, time:new Date('1981')}, function(er, latest) {
    ...
    var timed_out = function() {
      LOG.debug("Too few changes within " + max_age + "ms; updating: " + clean_db);
    ...
    heater_state.timer = setTimeout(timed_out, max_age); // XXX: Punting on the time_since_query dance as above.
  })

/*
      couch_changes.follow({db:opts.db, heartbeat:55000, include_docs:false}, function(change) {
        LOG.debug("GOt a change");
      })
    })
  } */
}
      ...
      forEach_ddoc(db, function(er, result, a, all_ddocs) {
        ...
        if(view_names.length == 0) {
          ...
          lib.req_json({uri:view}, function(er, res, body) {
            ...
            LOG.debug('View "' + ddoc_name + '/_view/' + view_name + '" is at least up to seq ' + db_update_seq);
            return completed_ddoc(ddoc, all_ddocs);
          })
        }
      })
