    ...
    lib.req_couch({uri:uri}, function(er, res, body) {
      try {
        ...
        body.forEach(function(db_name) {
          var db_url = lib.join(couch, encodeURIComponent(db_name));
...
function heat_db(opts) {
  ...
  LOG.info("Heating views on " + clean_db + " at most " + max_age + "ms or " + max_seq + " changes (compacting at "+opts.compact+")");
  ...
  lib.req_json({uri:db}, function(er, res, body) {
    ...
    forEach_ddoc(db, function(er, result, a, all_ddocs) {
      ...
      if(view_names.length == 0) {
        ...
        var view_name = rnd_element(view_names);
        var view = [db, enc_id(ddoc._id), '_view', view_name, '?limit=1'].join('/');
